...
class TasksController {
  ...
  public async update(req: Request, res: Response): Promise<Response> {
    ...
    //Check the existance of the task
    let task: Task | null;
    ...
    //Return 400 if there is not such a task
    if (!task) {
    ...
    try {
      updatedTask = await AppDataSource.getRepository(Task).update(req.body.id, {status: req.body.status});
